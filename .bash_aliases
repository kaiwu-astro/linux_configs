...
alias _do_upgrade_bash_aliases='[ "$(hostname)" != "kstation" ] && ping -c 1 gitee.com &> /dev/null && mkdir -p ~/.kai_config && cd ~/.kai_config && rm -rf * &> /dev/null && wget -O ~/.kai_config/kai_config.zip https://gitee.com/kaiwu-astro/linux_configs/repository/archive/main.zip &> /dev/null && unzip -o kai_config.zip &> /dev/null && cd linux_configs-main && cp -pr .[!.]* ~/ && cp -pr * ~/ &> /dev/null'
